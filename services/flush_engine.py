...
class FlushEngineService:
    @staticmethod
    def get_wrong_suit_cards(card_in_hand: CardPile, best_suit: str) -> CardPile:
        other_cards = [card for card in card_in_hand.cards if card.suit != best_suit]
        return CardPile(other_cards)

    @staticmethod
    def get_legal_suit_with_most_cards_in_hand(card_in_hand: CardPile, legal_suits: Optional[List[str]] = None) -> str:
